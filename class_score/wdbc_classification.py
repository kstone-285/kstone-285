...
def load_wdbc_data(filename):
    ...
    with open(filename) as f:
        for line in f.readlines():
            ...
            wdbc.data.append([float(items[x]) for x in range(2,30)])         # TODO #1) Add 30 attributes (as floating-point numbers)
...
if __name__ == '__main__':
    ...
    # Train a model
    model = svm.SVC(kernel='rbf', gamma=4)                           # TODO #2) Find a better classifier (SVC accuracy: 0.902)
